# -*- coding: utf-8 -*-
"""
Created on Mon Jul 30 10:50:50 2018
...
curve fit for each pixel of the image
@author: shihao
"""
...
data_dir = r'D:\irimages\irholography\bead8\bead_holo'  #directory of all raw data
...
re_dir = r'D:\irimages\irholography\bead8\ref'
...
bgs_dir = r'D:\irimages\irholography\bead8\bg_holo'
...
def im_fit(D):
    fit_im = numpy.zeros((128, 128))
    for i in range(128):
        for j in range(128):
            Dx_index = i
            Dy_index = j
            D_pixel = D[:, Dx_index, Dy_index]
            popt_D = fit_(D_pixel)
            fit_im[i,j] = popt_D[0]
...
fit_im_D = im_fit(D)
fit_im_D_sub = im_fit(D_sub)

plt.figure()
plt.subplot(121)
plt.imshow(fit_im_D)
plt.colorbar()
plt.title('D Matrix Fit Image')

plt.subplot(122)
plt.imshow(fit_im_D_sub)
plt.colorbar()
plt.title('D_sub Matrix Fit Image')

plt.suptitle('Curve Fits for Each Pixel', fontsize = 15)
plt.show()
