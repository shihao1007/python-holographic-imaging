# -*- coding: utf-8 -*-
"""
Created on Mon Jul 23 12:00:53 2018
...
#_min, _max = numpy.amin(image_seq_sub), numpy.amax(image_seq_sub)    
#
#
...
#img = []
#for i in range(positions):
#    img.append([plt.imshow(image_seq_sub[:, :, i], vmin = _min, vmax = _max)])
#
...
start = 5                   #the starting point of sampling from the raw dataset
...
def cal_field(num, start, image_seq):
    ...
    interval = int(positions / num)
    
    
    #D matrix denoting the difference between consecutive measuremnts
    D = [(image_seq[:,:,start + i*interval + 1] - image_seq[:,:,start + i*interval]) for i in range(num)]
...
def plot_fields():
    ...
    for i in range(5):
        num_pri = num * (2 ** i)
        F_raw = cal_field(num_pri, start, image_seq_sub)

        plt.subplot(2,3,i+1)
        plt.imshow(numpy.abs(F_raw))
        plt.title(str(num_pri) + ' measurements')
        plt.colorbar()
        
    num_last = positions - 1 - start
    F_raw = cal_field(num_last, start, image_seq_sub)
    ...
    plt.suptitle('Sub Fields', fontsize = 15)
    
F_sub = cal_field(12, 5, image_seq_sub)
#    
plt.figure()
plt.imshow(numpy.imag(F_sub))
#plt.title('Magnitude')
#plt.colorbar()
#plt.axis('off')

plt.figure()
plt.imshow(image_seq_raw[...,5])
