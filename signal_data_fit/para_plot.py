...
def cal_D(num):
    ...
    D = [(image_seq_raw[64,64,start + i + 1] - image_seq_raw[64,64,start + i]) for i in range(num)]
    D = numpy.asarray(D)
...
def cal_PH_P_Ph(num):
    ...
    #linear P matrix term before D matrix when calculating interferometric cross terms u
    PH_P_Ph =  PH_P_inv * cal_PH(num)
    PH_P_Ph_alpha = PH_P_Ph[0,:]
    PH_P_Ph_alpha = PH_P_Ph_alpha.reshape((199,1))

    return PH_P_Ph_alpha + 0j
...
F_a = numpy.zeros(numpy.size(num)) + 0j
...
#D_single = cal_D(num_x)
#P_term = cal_PH_P_Ph(num_x)
###P_term = pass
#
#plt.figure()
#
#plt.subplot(211)
#plt.plot(x, D_single,'r')
#plt.xlabel('Mirror Positions')
#plt.ylabel('D Matrix Value')
#
#plt.subplot(212)
#plt.plot(x, numpy.real(P_term),'b')
#plt.xlabel('Mirror Positions')
#plt.ylabel('P_term Value')
#
#plt.suptitle('P_term and D Matrix at 200 Measurements')
#
##
##plt.subplot(311)
##plt.plot(num, numpy.abs(F_a), 'r')
###plt.text(100, 30, 'D Matrix')
###plt.xlabel('Number of Measurements')
##plt.ylabel('Absolute Value')
##
##plt.subplot(312)
##plt.plot(num, numpy.real(F_a), 'b')
###plt.text(100, 0.20,'Hermitian P Matrix')
###plt.xlabel('Number of Measurements')
##plt.ylabel('Real Part')
##
##plt.subplot(313)
##plt.plot(num, numpy.imag(F_a), 'g')
###plt.text(100, 0.05,'PH_P_PH term')
##plt.xlabel('Number of Measurements')
##plt.ylabel('Imaginary Part')
##
##plt.suptitle('Convergence of u Matrix', fontsize = 15)
#plt.show()
